...
y3 <- left_join(x, y) %>%
  mutate(prop_hosp = value_hosp/value*100)
y3$index <- scenario_num$scenario[i]

df_out[[i]] <- y3
}
mm <- df_out
x <- do.call(rbind, df_out)

df_out <- left_join(x, scenarios) %>%
  filter(vacc_on == 1) %>%
  mutate(mu_ab_d1 = round(mu_ab_d1, 2)) %>%
  filter(mu_ab_d1 == 0.43) %>%
  group_by(age) %>%
  summarise(prop_hosp = mean(prop_hosp)) %>%
  ungroup()
df_out
