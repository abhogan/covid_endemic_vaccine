...
for (i in 1:length(df_all)){
  saf_reps_summarise <- df_all[[i]] %>%
    ...
    select(timestep, incidence, hosp_all, vax_ab_mean, nat_ab_mean, nat_ab_upper, nat_ab_lower, nat_mean, nat_lower, nat_upper, vax_ab_lower, vax_ab_upper, vax_ab_mean, IMild_count, ICase_count, E_count, tidyselect::contains("X", ignore.case=F)) %>%
    pivot_longer(cols = c(incidence, hosp_all, vax_ab_mean, nat_ab_mean, nat_ab_upper, nat_ab_lower, nat_mean, nat_lower, nat_upper, vax_ab_lower, vax_ab_upper, vax_ab_mean, IMild_count, ICase_count, E_count, tidyselect::contains("X", ignore.case=F)), names_to = "compartment") %>%
  ...
  saf_reps_summarise <- saf_reps_summarise %>%
    mutate(doses = X1_count + X2_count*2,
...
# ## dont summarise totals over repetitions
# df_totals_reps <- df %>%
#   # scale outputs to per-million-population
#   select(-cols) %>%
#   mutate(inc = inc / target_pop * 1e6,
...
#   ungroup() 
# 
# saveRDS(df_totals_reps, paste0("../covid_katana_runs/processed_outputs/df_summarise_totals_", name, "_reps.rds"))
