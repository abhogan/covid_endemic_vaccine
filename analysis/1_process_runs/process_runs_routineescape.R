...
for (i in 1:length(df_all)){
  ind <- scenario_num$scenario[i]
  start_escape <- filter(scenarios, index == ind)
  start_sum <- start_escape$vfr2_time1[1]
  if (timeframe == "12m") {
  end_sum <- start_sum + 365
  } else {
    start_sum <- start_escape$vfr2_time1[1] + 90
    end_sum <- start_sum + 183
  }
...
if (timeframe == "12m") {
  ...
  saveRDS(df_summarise, paste0("../covid_katana_runs/processed_outputs/df_summarise_", name, "_6m.rds"))
  saveRDS(df_totals, paste0("../covid_katana_runs/processed_outputs/df_summarise_totals_", name, "_6m.rds"))
}
