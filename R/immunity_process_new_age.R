...
run_model_new_age <- 
  function(){
    ...
    rt_out <- rt_out_sub * (1 + 0.05 * cos(2 * pi * c(1:time_period) / 365))
    ...
    # incidence_renderer <- individual::Render$new(parameters$time_period)
    # attach_tracking_listener_age_incidence(
    #   events = events, renderer = incidence_renderer,
